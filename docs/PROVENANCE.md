...
The initial **21 selected fresh source files** were physically materialized outside Git and blob-verified before adaptation. Reviewer A's retention-first repair adds `scripts/check_authority_boundary.py` and `scripts/run_heavy_validation.sh` to the selected set; those two pinned files were separately materialized and verified before adaptation, for **23 selected fresh files total**. Initial staging inventory SHA-256 remains `861395ff5961602ea985e52cbff46f928a074283e1cfdaa1236994d11fcf2512`; the two-file repair staging inventory SHA-256 is `28ab7c99782277b43196f5036920468edf1c0681dd7d649e09da2297d77427f1`. Raw unsanitized source material was not committed.

Sixteen retained files remain literal/byte-identical. Seven mixed files are minimally modified: the Kustomize base resource edge is redirected to the retained namespace; Helm values drop application image/service/agent/resource sections while keeping namespace/observability/mesh toggles; the deployment validator drops application composition checks and keeps platform structure validation; the backend parser test uses synthetic coordinates and imports the extracted pure helper; the mixed backend bootstrap contributes only its pure bucket-contract normalization/comparison logic; `scripts/run_heavy_validation.sh` retains source `phase_kubernetes` Helm/Kustomize rendering plus `phase_terraform` fmt/init/validate/native-test structure while removing application phases, Skaffold orchestration, token transport and Terraform plan behavior; and `scripts/check_platform_authority.py` retains source scan-root/prohibited-pattern/backend/resource/fail-closed algorithms while replacing Project 03/Ops allowlists and live backend coordinates with destination-owned bounds.

`scripts/select_heavy_validation.py` remains deliberately unretained because destination CI unconditionally runs the retained Terraform and Kubernetes heavy phases on every PR/push. Its Travis path selector would reduce coverage and its remaining phase routing is application-specific.
