...
During the current extraction phase, `Seemorghdev/edge-evidence-reference-platform` at source baseline `403ee070fcf31120c3f715bc32bb1643b428f7d2` **remains the live Project 03 authority** for active state, backend bindings, operational workflows, accepted cloud evidence, the existing external address, and current WIF/trust relationships. This repository is a sanitized copy/generalization target only.

Repository approval does **not** grant authority for cloud authentication, Terraform plan/apply/import against live state, state migration, WIF trust consumption/migration, Project 03 cutover, address adoption/promotion, DNS/TLS changes, or provider mutation.
...
No live Project 03 coordinates, retained external IPv4, state/plan/import material, address adoption evidence, live WIF pool/provider/service-account/repository coordinates, WIF consumption workflow, provider authentication, operational workflows, Kubernetes workloads/Ingress/Service/NEG, DNS/TLS/certificates, or cloud execution are present.

A later explicit authority-cutover review is required before this repository can become canonical for existing live state or trust. Operations/execution governance and WIF consumption semantics belong to the separate Operations product.
